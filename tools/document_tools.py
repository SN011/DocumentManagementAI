...
class GoogleDocWriteTool(BaseTool):
    ...
    def append_text_with_formatting(self, document_id, text, font_name, font_size, bold, italic, alignment, color, start_index):
        print('INPUT TEXT TO APPEND TEXT WITH FORMATTING FUNCTION:',text)
        if not text.strip():
        ...
        new_start_index = start_index + len(text)
        print(f'New start index after appending text: {new_start_index}')  # Debug print
